...
def stream_llm_forward(n_local, n_init, fattn: bool = False, *args, **kwargs):
    ...
    def forward(
        ...
        if past_key_value is not None:
            # concatenate old k, v with current k, v
        ...
        if use_cache:
            if len_k <= n_local + n_init:
                ...
                # cut initial and local data for cache
        ...
        # 从 K 和 V 张量中裁剪出与当前查询相关的部分，确保注意力机制只在必要的序列长度上计算，以减少不必要的计算负担并优化性能。
        ...
        # 检查键的长度是否大于局部窗口大小。如果是，说明需要额外处理用于注意力机制的初始化区域
        if len_k > n_local:
