...
## Changes Made
 - This code incorporates support for the Q-LLM patch.

## Usage 
```bash
python -m qllm.chat \
    --model-path models/Mistral-7B-Instruct-v0.2 \
    --qllm-config-path config/llama3-qllm-repr4-l1k-bs128-topk8-w4.yaml
```
...
from qllm.utils import patch_hf
...
def generate_stream(
    ...
    for i in range(max_new_tokens):
        if i == 0:  # prefill
            # TODO: auto extract question_ids from input_ids
            out = model(input_ids=start_ids, use_cache=True, past_key_values=past_key_values, question_ids=None)
            logits = out.logits
...
def chat_loop(
    ...
    qllm_config: Optional[dict] = None,
    revision: str = "main",
    ...
    if qllm_config is not None:
        model = patch_hf(model, qllm_config.type,  **qllm_config)
...
def main(args):
    ...
    if args.qllm_config_path is not None:
        from omegaconf import OmegaConf
        qllm_config = OmegaConf.load(args.qllm_config_path)["model"]
    else:
        qllm_config = None
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--qllm-config-path",
        type=str, help="Q-LLM patch config",
        default=None
    )
    parser.add_argument(
        "--clear-kv-cache",
